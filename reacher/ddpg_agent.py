...
class Agent():
    ...
    def step(self, states, actions, rewards, next_states, dones, timestep):
        ...
        if timestep > 0 and len(self.memory) > BATCH_SIZE and timestep % LEARN_EVERY == 0:
            #print(f"training at {timestep} for 10 batches.")
            for _ in range(TRAIN_STEPS):
                experiences = self.memory.sample()
                self.learn(experiences, GAMMA)

    def step2(self, state, action, reward, next_state, done, timestep):
        """Save experience in replay memory, and use 
        random sample from buffer to learn.
        """
        # Save experience / reward
...
class ReplayBuffer:
    ...
    def __len__(self):
        ...
        return len(self.memory)  # min(self.batch_size, self.count)
